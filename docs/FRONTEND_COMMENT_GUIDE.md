 ...
 * 評価ステータス
 *
 * 禁止: バックエンドでは int（0,1,2）として保存されるため、
 *       順序の変更・値の再割当を行わないこと
 * 新しい状態は必ず末尾に追加する
...
`/** ユーザーを取得する */` のような、シグネチャを日本語訳しただけのコメントは通過する
